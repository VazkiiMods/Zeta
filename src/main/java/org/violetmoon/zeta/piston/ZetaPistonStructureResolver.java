...
public class ZetaPistonStructureResolver extends PistonStructureResolver {
	...
	public boolean resolve() {
		...
		if (!PistonBaseBlock.isPushable(blockstate, world, blockToMove, moveDirection, false, moveDirection)) {
			...
			// Changed iteration to an index-based loop to safely handle modifications to myToPush
			int i = 0;
			while (i < myToPush.size()) {
		...
		if (state.isAir()
			...
			if (lineLen + myToPush.size() > max)
				...
				while (true) {
					...
					// Vanilla logic
					if (state.isAir() || !(oldState.canStickTo(state) && state.canStickTo(oldState))
							|| !PistonBaseBlock.isPushable(state, world, target, moveDirection, false, moveDirection.getOpposite())
							|| target.equals(pistonPos))
						break;
				...
				while (true) {
					...
					if (!PistonBaseBlock.isPushable(state, world, currentPos, moveDirection, true, moveDirection)
							|| currentPos.equals(pistonPos))
						return false;
