...
public class DyeablesRegistry {
	...
	class WashingInteraction implements CauldronInteraction {
		...
		@Override
		public @NotNull InteractionResult interact(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, ItemStack stack) {
			if(!isDyed(stack))
	...
	public DyedItemColor getColor(ItemStack stack) {
		return isDyed(stack) ? stack.getComponents().get(DataComponents.DYED_COLOR) : new DyedItemColor(0xFF_FF_FF);
	}
	...
	public ItemStack dyeItem(ItemStack stack, List<DyeItem> dyes) {
		...
		if(isDyeable(stack)) {
			...
			int j2 = (j1 << 8) + k1;
			j2 = (j2 << 8) + l1;
			stack.set(DataComponents.DYED_COLOR, new DyedItemColor(j2));
